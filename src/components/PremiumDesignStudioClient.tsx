...
const PRODUCTS = [
  { id: "tshirt" as const, label: "T-Shirt", base: 230, min: 10, lead: "5–7 working days" },
  { id: "polo" as const, label: "Polo Shirt", base: 310, min: 10, lead: "6–8 working days" },
  { id: "hoodie" as const, label: "Hoodie", base: 690, min: 8, lead: "7–10 working days" },
];
          ...
          <form onSubmit={submitQuote} className="grid min-w-0 gap-5 lg:grid-cols-[minmax(0,1.35fr)_minmax(330px,.8fr)]">
            ...
            <aside className="flex min-h-[600px] flex-col rounded-[26px] border border-[#dfded8] bg-[#fff] shadow-[0_18px_55px_rgba(32,30,24,0.07)]">
              ...
              <motion.div key={step} initial={{ opacity: 0, x: 8 }} animate={{ opacity: 1, x: 0 }} className="flex-1 p-5">
                {step === 1 ? <div className="grid gap-3 sm:grid-cols-3 lg:grid-cols-1">{PRODUCTS.map((option) => <button key={option.id} type="button" onClick={() => setProductId(option.id)} className={`relative flex items-center gap-4 rounded-2xl border p-3 text-left transition ${productId === option.id ? "border-[#ff5a0a] bg-[#fff8f3] ring-2 ring-[#ff5a0a]/10" : "border-[#e2e1dc] hover:border-[#bbb9b1]"}`}><span className="flex h-24 w-20 shrink-0 items-center justify-center overflow-hidden rounded-xl bg-[#f1f0ec] p-2"><Garment productId={option.id} side="front" colorHex="#111318" /></span><span><span className="block text-sm font-extrabold">{option.label}</span><span className="mt-1 block text-xs text-[#7a7972]">From {formatMoney(option.base)}</span><span className="mt-2 block text-[9px] font-bold uppercase tracking-[0.1em] text-[#a09e96]">{option.lead}</span></span>{productId === option.id ? <CheckCircle2 className="absolute right-3 top-3 h-5 w-5 text-[#ff5a0a]" /> : null}</button>)}</div> : null}
