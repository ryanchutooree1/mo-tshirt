...
export default function ShopClient() {
  ...
  function addLineItem(id: string) {
    setSelections((prev) => {
      ...
      const existingIndex = nextLines.findIndex(
        (entry) => entry.color === line.color && entry.size === line.size
      );
      if (existingIndex >= 0) {
        nextLines[existingIndex] = {
          ...nextLines[existingIndex],
          quantity: nextLines[existingIndex].quantity + line.quantity,
        };
      } else {
        nextLines.push(line);
      }
  ...
  function removeLineItem(id: string, index: number) {
    setSelections((prev) => {
      const current = prev[id];
      if (!current) return prev;
      return {
  ...
  function updateLineQty(id: string, index: number, quantity: number) {
    setSelections((prev) => {
      const current = prev[id];
      if (!current) return prev;
      const nextLines = current.lines.slice();
  ...
  function clearLineItems(id: string) {
    setSelections((prev) => {
      const current = prev[id];
      if (!current) return prev;
      return {
      ...
      <main className="mx-auto w-full max-w-6xl px-6 py-14">
        ...
        <section className="mt-10 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {filtered.map((item) => {
            ...
            const orderLines = selection.lines.length
              ? selection.lines
              : [
                  {
            ...
            const orderMessage = buildShopWhatsAppMessageForLines(
              item,
              orderLines,
                ...
                <div className="mt-4 space-y-3">
                  ...
                  <div className="flex flex-wrap items-center gap-2">
                    <button
                      ...
                      className="inline-flex items-center justify-center rounded-full border border-black px-4 py-2 text-xs font-semibold text-black transition hover:bg-black hover:text-white"
                    >
                      Add to order
                    </button>
                    {!!selection.lines.length && (
                      <button
                        type="button"
                        onClick={() => clearLineItems(item.id)}
                        className="inline-flex items-center justify-center rounded-full border border-neutral-300 px-4 py-2 text-xs font-semibold text-neutral-600 transition hover:bg-neutral-100"
                      >
                        Clear list
                      </button>
                    )}
                  ...
                  <div className="rounded-2xl border border-neutral-200 bg-neutral-50 p-3">
                    <div className="flex items-center justify-between">
                      <p className="text-xs font-semibold text-neutral-700">Order list</p>
                      <span className="text-[11px] text-neutral-500">
                        {selection.lines.length} item{selection.lines.length === 1 ? "" : "s"}
                      </span>
                    </div>
                    <div className="mt-3 space-y-2 text-xs text-neutral-700">
                      {selection.lines.length ? (
                        selection.lines.map((line, lineIndex) => (
                          <div
                            key={`${line.color}-${line.size}-${lineIndex}`}
                          ...
                          >
                            ...
                            <div className="flex items-center gap-2">
                              <input
                                ...
                                value={line.quantity}
                                onChange={(e) =>
                                  updateLineQty(item.id, lineIndex, Number(e.target.value || 1))
                                }
                                className="w-16 rounded-xl border border-neutral-200 bg-white px-2 py-1 text-xs"
                              />
                              <button
                                type="button"
                                onClick={() => removeLineItem(item.id, lineIndex)}
                        ...
                        <p className="text-xs text-neutral-400">
                          Add multiple colors or sizes to build your WhatsApp order.
                        </p>
                      )}
                    </div>
                  </div>
