...
export default function InboxPage() {
  ...
  return <main className="mx-auto max-w-7xl p-4 text-gray-900 sm:p-8">
    ...
    <Link href="/admin/quotation-approval" className="mb-5 flex items-center justify-between rounded-xl bg-emerald-50 p-4 text-sm font-semibold text-emerald-900">All client requests · Quotes &amp; invoices <span>Open →</span></Link>
    {canReconnect && <div className="mb-4 flex flex-wrap items-center justify-between gap-2 text-sm"><span className="text-gray-500">{connectionNotice || "Gmail connection managed securely by the website."}</span><button type="button" onClick={() => window.location.assign("/api/admin/inbox/oauth")} className="font-semibold text-emerald-800">Reconnect Gmail</button></div>}
