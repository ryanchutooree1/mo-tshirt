...
All requests appear in one list at `/admin/quotation-approval`: quote form, design studio, email and existing staff-entered WhatsApp requests. Source chips filter this same list; they do not open separate queues. `/admin/inbox/enquiries` redirects here and preserves an enquiry’s selected id. Client conversations from the last 90 days are checked using read-only Gmail access. Groq’s GPT-OSS 120B model extracts grounded details in English or French. Website quotation copies, automatic notifications and supplier pitches are excluded. Ambiguous buying intent stays in Review.
...
Complete enquiries create one `quotes/gmail-{threadId}` document, preserving the conversation and leaving prices empty for the owner. An existing quote is never overwritten. Incomplete enquiries remain in `emailIntake` until client replies provide the missing details, but are displayed alongside quotes with Needs details / Waiting for client stages. The shared list merges by thread-derived id: when a quote is created it replaces the enquiry row without duplicates. The selected email opens its details, original conversation and exact Send questions preview directly in the quote workspace. Inbox and Quotes permissions are still both required to load email enquiries. After a quote is created, subsequent messages remain available in Inbox; they do not change the quote automatically.
