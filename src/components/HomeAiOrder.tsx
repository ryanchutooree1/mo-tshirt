...
export default function HomeAiOrder() {
  ...
  return (
    <section className="mx-auto w-full max-w-[54rem]">
      <div className="relative overflow-hidden rounded-[30px] border border-[#f0e5fb] bg-[linear-gradient(180deg,rgba(255,255,255,0.98)_0%,rgba(255,248,252,0.98)_46%,rgba(248,252,255,0.98)_100%)] p-3 shadow-[0_30px_80px_-56px_rgba(142,82,255,0.42)] sm:rounded-[36px] sm:p-5 sm:shadow-[0_36px_90px_-58px_rgba(142,82,255,0.42)]">
        <div
          aria-hidden
        ...
        <div className="relative rounded-[24px] border border-white/90 bg-white/[0.9] p-3 shadow-[0_20px_56px_-42px_rgba(124,58,237,0.28)] backdrop-blur sm:rounded-[30px] sm:p-5 sm:shadow-[0_24px_66px_-46px_rgba(124,58,237,0.32)]">
          <div className="flex flex-col items-start gap-3 border-b border-[#eee7f7] pb-4 sm:flex-row sm:flex-wrap sm:items-start sm:justify-between">
            <div className="min-w-0 flex-1">
              <div className="inline-flex items-center gap-2 rounded-full border border-[#eadcff] bg-[linear-gradient(135deg,rgba(34,211,238,0.12),rgba(168,85,247,0.12),rgba(244,114,182,0.12))] px-3 py-1 text-[10px] font-semibold uppercase tracking-[0.22em] text-[#6b4ccf] sm:text-[11px] sm:tracking-[0.24em]">
                <Bot className="h-3.5 w-3.5 text-[#8257ff]" />
                Live order assistant
              </div>
              <h3 className="mt-3 text-[1.55rem] font-semibold leading-[1.08] tracking-tight text-[#161224] sm:text-[2rem]">
            ...
            {hasMessages && (
              <button
                ...
                className="inline-flex w-full items-center justify-center gap-2 rounded-full border border-[#e8dcff] bg-white px-4 py-2 text-sm font-medium text-[#5f4db8] transition hover:border-[#cfb9ff] hover:bg-[#fbf8ff] sm:w-auto"
              >
          ...
          <div
            ref={messageListRef}
            className="mt-4 flex max-h-[18.5rem] flex-col gap-3 overflow-y-auto rounded-[22px] border border-[#efe8fa] bg-[linear-gradient(180deg,rgba(255,255,255,0.78),rgba(252,247,255,0.92))] p-3 shadow-[inset_0_1px_0_rgba(255,255,255,0.7)] sm:rounded-[26px] sm:p-5"
          >
            {hasMessages ? (
              <>
                {session.messages.map((entry) => {
                  ...
                  return (
                    <div key={entry.id} className={`flex ${userMessage ? "justify-end" : "justify-start"}`}>
                      <div
                        className={`max-w-[92%] rounded-[20px] border px-4 py-3 shadow-[0_20px_38px_-32px_rgba(74,39,150,0.22)] sm:max-w-[85%] sm:rounded-[22px] ${
                          userMessage
                ...
                {canUploadLogo && !session.lead.logoAttachment && !hasAssistantLogoPrompt ? (
                  <div data-thread-item className="flex justify-start">
                    <div className="max-w-[92%] rounded-[20px] border border-[#ede6fb] bg-white/95 px-4 py-3 text-[#231f38] shadow-[0_20px_38px_-32px_rgba(74,39,150,0.22)] sm:max-w-[85%] sm:rounded-[22px]">
                ...
                {canUploadLogo && !session.lead.logoAttachment ? (
                  <div data-thread-item className="flex justify-end">
                    <div className="max-w-[92%] rounded-[20px] border border-[#eadbff] bg-[linear-gradient(135deg,#fff8fe_0%,#eefaff_52%,#fff6eb_100%)] px-4 py-3 text-[#1d1831] shadow-[0_20px_38px_-32px_rgba(74,39,150,0.22)] sm:max-w-[85%] sm:rounded-[22px]">
              ...
              <div className="flex min-h-[13rem] flex-col items-center justify-center rounded-[20px] border border-dashed border-[#e8defa] bg-[linear-gradient(135deg,rgba(255,255,255,0.95),rgba(246,250,255,0.92),rgba(255,248,252,0.95))] px-4 py-5 text-center sm:rounded-[22px] sm:px-5 sm:py-6">
                <div className="relative flex h-14 w-14 items-center justify-center rounded-full border border-[#eadcff] bg-[linear-gradient(135deg,rgba(34,211,238,0.14),rgba(168,85,247,0.16),rgba(244,114,182,0.14))] shadow-[0_18px_44px_-30px_rgba(124,58,237,0.45)] sm:h-16 sm:w-16">
                  <div
                    aria-hidden
                    className="absolute inset-2 rounded-full bg-[linear-gradient(135deg,rgba(255,255,255,0.95),rgba(252,246,255,0.92))]"
                  />
                  <Bot className="relative h-6 w-6 text-[#7a5de1] sm:h-7 sm:w-7" />
                </div>
                <p className="mt-4 text-base font-semibold leading-6 text-[#211c33]">Start with a real client-style message</p>
                ...
                <div className="mt-4 grid w-full gap-2 sm:flex sm:flex-wrap sm:justify-center">
                  {STARTER_MESSAGES.map((starter) => (
                    <button
                      key={starter}
                      type="button"
                      onClick={() => applyStarterMessage(starter)}
                      className="w-full rounded-2xl border border-[#e3d6fb] bg-white px-4 py-3 text-center text-[13px] font-semibold leading-5 text-[#5d4bb6] transition hover:border-[#cdb9ff] hover:bg-[#faf7ff] sm:w-auto sm:rounded-full sm:px-3 sm:py-2 sm:text-xs sm:leading-normal"
                    >
                      {starter}
                    </button>
                  ))}
          ...
          <form ref={formRef} onSubmit={handleSubmit} className="mt-4">
            <div className="rounded-[22px] border border-[#ece4fb] bg-[linear-gradient(180deg,rgba(255,255,255,0.92),rgba(252,248,255,0.95))] p-3 shadow-[0_20px_44px_-34px_rgba(124,58,237,0.22)] sm:rounded-[26px] sm:p-4 sm:shadow-[0_24px_48px_-38px_rgba(124,58,237,0.25)]">
              ...
              <div className="mt-4 flex flex-col gap-3 sm:flex-row sm:flex-wrap sm:items-center sm:justify-between">
                ...
                <div className="flex flex-col gap-2 sm:items-end">
                  {pendingLogoFile ? (
                    <span className="text-xs font-medium text-[#8a7db8] sm:text-right">
                  ...
                  <button
                    ...
                    className="inline-flex w-full items-center justify-center gap-2 rounded-full bg-[linear-gradient(135deg,#22d3ee_0%,#8b5cf6_48%,#ec4899_100%)] px-5 py-3 text-sm font-semibold text-white shadow-[0_22px_40px_-24px_rgba(124,58,237,0.55)] transition hover:brightness-110 disabled:cursor-not-allowed disabled:opacity-50 sm:w-auto"
                  >
