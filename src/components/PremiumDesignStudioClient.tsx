...
const STEPS: { id: StudioStep; label: string; short: string }[] = [
  ...
  { id: 2, label: "Choose colour", short: "Colour" },
  { id: 3, label: "Sizes & quantities", short: "Sizes" },
  { id: 4, label: "Choose print area", short: "Print area" },
  { id: 5, label: "Upload artwork", short: "Artwork" },
  { id: 6, label: "Position design", short: "Position" },
  { id: 7, label: "Add custom text", short: "Text" },
  { id: 8, label: "Review order", short: "Review" },
  { id: 9, label: "Request quote", short: "Quote" },
];
...
function getShopThumbnailUrl(src: string) {
  if (!src.includes("/api/shops/uploads/") || src.includes("variant=thumbnail")) return src;
...
export default function PremiumDesignStudioClient({
  ...
  const upperPlacement = activeSide === "front"
  ...
  const totalPrice = productionSubtotal + minimumOrderAdjustment;
  ...
  return (
    ...
    >
      ...
      <section data-studio-part="heading" className="hidden border-b border-[#e5e4df] bg-[#fff] px-4 py-5 sm:block sm:px-6 sm:py-6 xl:shrink-0 xl:py-3">
        <div className="mx-auto max-w-[1500px]">
          <div data-studio-part="progress-row" className="flex flex-col justify-between gap-4 xl:flex-row xl:items-end">
            <HeadingTag data-studio-part="title" className="text-3xl font-bold leading-tight tracking-[-0.045em] text-[#161613] [font-family:var(--font-studio-display)] sm:text-4xl xl:text-3xl">Design it. <span className="text-[#ff5a0a]">We print it.</span></HeadingTag>
          ...
          <div data-studio-part="steps" className="mt-6 hidden gap-2 overflow-x-auto pb-1 sm:flex xl:hidden">{STEPS.map((item) => <button key={item.id} type="button" aria-current={step === item.id ? "step" : undefined} onClick={() => setStep(item.id)} className={`flex shrink-0 items-center gap-2 rounded-full border px-3 py-2 text-xs font-bold ${step === item.id ? (appearance === "editorial" ? "text-[#181613]" : "studio-primary border-[#ff5a0a] bg-[#ff5a0a] !text-white") : step > item.id ? "border-[#ffd5c1] bg-[#fff5ef] text-[#c54306]" : "border-[#e4e3dd] bg-[#fff] text-[#73726c]"}`}><span>{step > item.id ? <Check className="h-3.5 w-3.5" /> : item.id}</span>{item.short}</button>)}</div>
        </div>
      </section>

      <div data-studio-part="body" className={`mx-auto max-w-[1500px] px-0 py-0 sm:h-auto sm:px-6 sm:py-7 ${embedded ? "h-[100dvh] xl:h-auto xl:min-h-0 xl:flex-1 xl:py-4" : "h-[calc(100dvh-69px)]"}`}>
        <div data-studio-part="layout" className="grid h-full items-start gap-0 sm:h-auto sm:gap-5 xl:h-full xl:min-h-0 xl:items-stretch xl:grid-cols-[230px_minmax(0,1fr)]">
          <nav data-studio-part="sidebar" className="hidden rounded-[24px] border border-[#e1e0da] bg-[#fff] p-3 shadow-[0_14px_45px_rgba(29,27,20,0.05)] xl:block xl:h-full xl:min-h-0 xl:overflow-y-auto" aria-label="Design steps">
          ...
          <form onChangeCapture={() => journey.start(STEPS.find(item => item.id === step)?.label)} onClickCapture={() => journey.start(STEPS.find(item => item.id === step)?.label)} data-studio-part="form" onSubmit={submitQuote} className="relative grid h-full min-w-0 gap-0 overflow-hidden sm:h-auto sm:gap-5 sm:overflow-visible lg:grid-cols-[minmax(0,1.35fr)_minmax(330px,.8fr)] xl:h-full xl:min-h-0 xl:overflow-hidden">
            ...
            <section data-studio-part="preview" className="absolute inset-0 flex min-h-0 flex-col overflow-hidden border-y border-[#dfded8] bg-[#fff] pb-[78px] shadow-none sm:static sm:block sm:rounded-[26px] sm:border sm:pb-0 sm:shadow-[0_18px_55px_rgba(32,30,24,0.07)] xl:flex xl:h-full">
              ...
              <div data-studio-part="stats" className="grid grid-cols-4 border-t border-[#ecebe6] bg-[#fbfbf9]"><PreviewStat label="Garment" value={product.label} /><PreviewStat label="Colour" value={selectedColor} /><PreviewStat label="Quantity" value={String(totalQty)} /><PreviewStat label="Estimate" value={formatMoney(totalPrice)} accent /></div>
            </section>
                ...
                <>
                  ...
                  {mobileTool === "text" ? (
                    <MobileWorkspace eyebrow="Text" title="Add custom text" description={`Editing the ${activeSide} side. Changes appear immediately on the shirt.`}>
                      ...
                      <div className="grid grid-cols-3 gap-2"><PresetButton icon={<Crosshair />} label={upperPlacement.label} onClick={() => patchText(upperPlacement.text)} /><PresetButton icon={<Focus />} label="Centre" onClick={() => patchText({ x: 0, y: 0 })} /><PresetButton icon={<Move />} label="Lower" onClick={() => patchText({ x: 0, y: 24 })} /></div>
            ...
            <aside data-studio-part="controls" className="mx-2 mb-4 hidden min-h-[520px] flex-col rounded-[26px] border border-[#dfded8] bg-[#fff] shadow-[0_18px_55px_rgba(32,30,24,0.07)] sm:mx-0 sm:mb-0 sm:flex sm:min-h-[600px] xl:h-full xl:min-h-0 xl:overflow-hidden">
              ...
              <motion.div data-studio-part="step-content" key={step} initial={{ opacity: 0, x: 8 }} animate={{ opacity: 1, x: 0 }} className="min-h-0 flex-1 overflow-y-auto p-5">
                {step === 1 ? <div className="space-y-3"><div data-studio-part="catalog-label" className="flex items-center gap-2 rounded-xl bg-[#f2f8f4] px-3 py-2 text-[10px] font-bold uppercase tracking-[0.1em] text-[#168455]"><BadgeCheck className="h-4 w-4" />{appearance === "editorial" ? "Available garments" : "Live products from Shops"}</div>{shopLoading ? <div className="flex min-h-48 items-center justify-center gap-2 text-xs font-semibold text-[#77766f]"><Loader2 className="h-4 w-4 animate-spin text-[#ff5a0a]" />Loading real garments…</div> : <div data-studio-part="products" className="grid gap-3 sm:grid-cols-2 lg:grid-cols-1">{PRODUCTS.map((option) => {
                  ...
                  return <button key={option.id} type="button" onClick={() => selectProduct(option.id)} aria-pressed={isSelected} disabled={shopLoading} className={`relative flex items-center gap-4 rounded-2xl border p-3 text-left transition disabled:cursor-wait disabled:opacity-45 ${isSelected ? "border-[#ff5a0a] bg-[#fff8f3] ring-2 ring-[#ff5a0a]/10" : "border-[#e2e1dc] hover:border-[#bbb9b1]"}`}><span className="relative flex h-24 w-20 shrink-0 items-center justify-center overflow-hidden rounded-xl bg-white"><LoadingImage src={preferredItem?.photoUrl || option.image} fallbackSrc={option.image} alt={`${option.label} from Shops`} wrapperClassName="h-full w-full" className="h-full w-full object-contain" loading={isSelected ? "eager" : "lazy"} fetchPriority={isSelected ? "high" : "auto"} decoding="async" delayMs={100} statusText="Loading…" compactStatus /></span><span className="min-w-0"><span className="block text-sm font-extrabold">{option.label}</span><span className="mt-1 block text-xs text-[#7a7972]">From {formatMoney(minimumPrice)}</span><span className="mt-2 block text-[9px] font-bold uppercase tracking-[0.1em] text-[#a09e96]">{variants.length ? `${variants.length} ${appearance === "editorial" ? "" : "shop "}colour${variants.length === 1 ? "" : "s"}` : "Standard fallback"}</span></span>{isSelected ? <CheckCircle2 className="absolute right-3 top-3 h-5 w-5 text-[#ff5a0a]" /> : null}</button>;
                ...
                {step === 2 ? (
                  <div data-studio-part="colour-selection" className="space-y-4">
                    <div className="flex items-center justify-between gap-3">
                      <Label>Available colours</Label>
                      <span className="text-xs font-semibold text-[#626a76]">{productVariants.length} colours</span>
                    </div>
                    {productVariants.length ? (
                      <div data-studio-part="colours" className="grid grid-cols-2 gap-3">
                        {productVariants.map((item) => {
                          ...
                          return (
                            <button
                              ...
                              className={`relative overflow-hidden rounded-2xl border p-3 text-left transition ${isSelected ? "border-[#ff5a0a] bg-[#fff8f3] ring-2 ring-[#ff5a0a]/10" : "border-[#d6dbe2] bg-white hover:border-[#929daa]"}`}
                            >
                              <span className="relative block aspect-square overflow-hidden rounded-xl bg-white">
                                <LoadingImage
                                  src={getShopThumbnailUrl(item.photoUrl || product.image)}
                                  fallbackSrc={product.image}
                                  alt={`${itemColor} ${item.title}`}
                              ...
                              <span className="mt-3 block text-xs font-bold">{itemColor}</span>
                              <span className="mt-1 block text-[11px] text-[#626a76]">From {formatMoney(getMinSizePrice(item))}</span>
                              {isSelected ? <CheckCircle2 className="absolute right-3 top-3 h-5 w-5 rounded-full bg-white text-[#ff5a0a]" /> : null}
                            </button>
                          );
                        })}
                      </div>
                    ) : (
                      <p className="rounded-xl bg-[#f5f6f8] p-4 text-xs leading-5 text-[#626a76]">
                        No live {product.label.toLowerCase()} colours are currently available. The standard preview is being used.
                      </p>
                    )}
                  </div>
                ) : null}
                  ...
                  <div data-studio-part="quantities" className="space-y-6">
                    <div data-studio-part="selected-garment" className="flex items-center gap-3 rounded-xl border border-[#d6dbe2] bg-[#f5f6f8] p-3">
                      <span className="h-16 w-14 shrink-0 overflow-hidden rounded-lg bg-white">
                        <LoadingImage
                          src={getShopThumbnailUrl(selectedShopItem?.photoUrl || product.image)}
                          fallbackSrc={product.image}
                          alt={`${selectedColor} ${product.label}`}
                          wrapperClassName="h-full w-full"
                          className="h-full w-full object-contain"
                          compactStatus
                        />
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        <p className="mt-1 text-xs text-[#626a76]">{selectedColor}</p>
                      </div>
                      <button type="button" onClick={() => setStep(2)} className="min-h-11 text-xs font-semibold underline underline-offset-4">
                        Change colour
                      </button>
                    </div>
                    <div>
...
function stepCopy(step: StudioStep) {
  return ["Start with the garment that fits your idea.", "Choose the colour for your garment.", "Enter how many you need in each size.", "Choose the side and best production method.", "Upload a high-quality logo or illustration.", "Resize, move and fine-tune the artwork.", "Add an optional name, message or team line.", "Check every detail before sending it.", "Tell us where to send the final price."][step - 1];
}
